...
/* 회원가입 JS 코드 작성 시 필요한 요소 모두 얻어오기 */
...
inputName.addEventListener("input",e =>{
  ...
  if(regExp.test(value)){
    span.textContent = "유효한 형식입니다";
    span.classList.add("check");
    span.classList.remove("error");
    checkList["inputName"] = true;

  } else{ //무효인 경우
    span.textContent = "유효하지 않은 형식입니다";
    span.classList.add("error");
    span.classList.remove("check");
    checkList["inputName"] =false;
  }
...
// /* 비밀번호 확인 유효성 검사 */ 
/* checkPwFn()이 아닌, 함수 이름만 작성하면 해당 부분에 
   함수 코드 그대로가 들어감 
   (콘솔에 찍어보기) */
  ...
  /* 아이디, 비밀번호, 비밀번호 확인, 이름이
    ...
    for(let key in checkList){
      ...
      if(checkList[key] ==false){//유효하지 않은 경우
        ...
        alert(`${str} 유효하지 않습니다`);
        e.preventDefault(); //form 기분 이벤트(제출) 막기
