...
document.querySelector("#btn2a").addEventListener("click",function(){
  ...
  /* 참조란?
    - 객체 지향 언어에서 
      ...
      저장된 곳의 주소를 저장해서 필요할 때마다 해당 주소를 찾아가
      원하는 값을 얻어오거나 수정할 수 있게 하는 것

      이때, 주소를 찾아가는 것을 [참조] 라고 한다!
  */
  arrayTest(arr1);
  arrayTest(arr2);
  ...
  //    -> 함수 수행 후 돌아 와도 참조하고 있던 2번 인덱스의
  //       값이 변화되어 있음.
  console.log(arr2[2]); //멜론 대신 바뀐 '딸기'가 나옴
});

/* 매개변수로 요소 전달하기 ---------------------------*/

function btn2bFn(el){
  //매개변수 el(element) : 이벤트가 발생한 요소(HTML에서 this) 담겨있음
...
function sumFn(arr){// 전달된 배열 요소의 합 반환
