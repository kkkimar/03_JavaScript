...
function check1(){
  ...
  */
  ...
  //배열명[index] = 값; -> 해당 인덱스에 지정된 값 대입 가능
  arr2[0] =100;
  arr2[1] ="눈온다";
  arr2[2] = true;
  // (JS 배열의 특징 : 인텍스별로 자료형을 다르게 할 수 있다
  //  단, 권장하지 않음 ! )
...
/* 주문하기 프로그램*/
/* 
  - 주문하기 버튼 클릭시 prompt를 이용해서 메뉴명,수량 입력 받기
  - 메뉴명 입력 시 취소 클릭하면 "주문완료"
  - 수량 입력 시 취소 클릭하면 해당 메뉴 주문만 취소
...
function orderFn(){
  ...
  //   counts[i] = 0;
  // }
  counts.fill(0); //비어있는 칸을 0으로 채움

  while(true){ //무한 반복

    const selectMenu = prompt("주문할 메뉴명을 입력(완료시 취소)");
    ...
    // 잘못 주분한 경우(idx == -1) 다시 반복 시작
    if(idx == -1) {
    alert("없는 메뉴 입니다 다시 입력하세요");
    continue; //while로 돌아가기 -> 다시 입력시작
    }
    ...
    if(input == null){//수량입력 취소 -> 다시 메뉴 입력
      continue; //while문으로 돌아가면 다시 입력시작됨
    }

    //수량을 입력하지 않았거나, 숫자를 입력하지 않은 경우
    if(input.length == 0 || isNaN(Number(input))){
      alert("잘못입력했습니다, 다시입력해주세요");
      continue;
    }
...
function check6(){
  ...
  for(let row = 0; row<5; row++ ){//행을제어

    //1차원 배열을 만들어 arr[i]에 대입
    arr[row] =[];

    for(let col =0; col <5; col++){//열을 제어
