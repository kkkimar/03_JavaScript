...
function classTest2(){
  ...
  //->HTMLCollection ( == 유사 배열 ) 형태로 얻어옴
  const inputList = document.getElementsByClassName("cls-test2");

  /* 중요!!!!!!!!!!!!!!!!!!!!!!!
     요소들을 한 번에 얻어오게 되면 배열 형태로 반환
    -> 요소를 다루고 싶으면 index를 이용해 
       배열 요소를 하나씩 꺼내서 다뤄야 한다 ! 
       ex)묶음 자체에 스타일 적용 또는 value값 얻기 안됨 !!! 
  */
...
function nameTest(){
  //name 속성값이 "hobby"인 요소를 모두 얻어와
  //hobbyList에 저장
  const hobbyList = document.getElementsByName("hobby");

  let str = "" //check된 값 누적
  let count = 0 //check된 수 카운트
  ...
  for(let i=0; i<hobbyList.length ; i++){
    ...
    if(hobbyList[i].checked){//true일 경우 실행
      count++;
...
function readValue(){
  ...
  //bg.scrollTop = 값 : 스크롤 윗부분 값 위치로 이동
  //(값이 너무 크면 제일 밑으로 이동)
